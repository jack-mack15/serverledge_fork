...
func findAreaPharos() (string, string, error) {
	...
	for _, anchor := range anchors {
		...
		if ((currRad == 0 && rtt.Milliseconds() <
			(int64)(config.GetInt(config.MAX_AREA_DISTANCE, 200))) ||
...
func monitorFailure() {
	checkTimer := time.NewTicker(time.Duration(config.GetFloat(config.DEAD_COLLECTIOR_INTERVAL, 6.0) * float64(time.Second)))
	for {
		select {
		case <-checkTimer.C:
			deadCollector()
